...
# 1. Check arguments
if [ $# -ne 2 ]; then
    echo "Usage: filesMove.sh <source_dir> <destination_dir>"
    exit 1
fi
...
if [ ! -d "$target_dir" ]; then
    echo "Destination folder '$target_dir' not found. Creating it..."
    mkdir "$target_dir"
fi
...
echo "Done: All files copied from '$src_dir' to '$target_dir' in alphabetically order."
